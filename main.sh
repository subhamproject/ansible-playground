...
sleep 10
[ $(sudo lxc ls -c ns --format csv ansible-client-$count|grep RUNNING|cut -f1 -d,|wc -l) -lt 1 ] && sudo lxc start ansible-client-$count </dev/null
echo " OK!"
done
}
...
sleep 10
CONTAINER=$1
[ $(sudo lxc ls -c ns --format csv $CONTAINER|grep RUNNING|cut -f1 -d,|wc -l) -lt 1 ] && sudo lxc start $CONTAINER </dev/null
